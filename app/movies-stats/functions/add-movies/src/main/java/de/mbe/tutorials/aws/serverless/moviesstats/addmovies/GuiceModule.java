package de.mbe.tutorials.aws.serverless.moviesstats.addmovies;

import com.google.inject.AbstractModule;
import de.mbe.tutorials.aws.serverless.moviesstats.addmovies.repositories.MoviesStatsDynamoDBRepository;
import de.mbe.tutorials.aws.serverless.moviesstats.addmovies.repositories.MoviesStatsRepository;
import de.mbe.tutorials.aws.serverless.moviesstats.addmovies.services.MoviesS3StorageService;
import de.mbe.tutorials.aws.serverless.moviesstats.addmovies.services.MoviesStorageService;

public final class GuiceModule extends AbstractModule {

    @Override
    protected void configure() {
        bind(MoviesStatsRepository.class).to(MoviesStatsDynamoDBRepository.class);
        bind(MoviesStorageService.class).to(MoviesS3StorageService.class);
    }
}
